...
class XBRLLinkBaseTree(IterableTree) :
	...
	def __init__(self, load_file_kind, xbrl_path_data) :
		...
		#definitionLinkまたはresentationLinkごとに構造を取得する
		document_number = 0
	...
	def read_instance_data(self, rol_id, xbrl_instance_file_analyzer, selected_axis_member_dict, target_time_str, one_before_str) :
		...
		#3次元以上のデータは処理できない
		if len(col_axis_member_list) > 1 :

			raise JPXAnalysisError('列方向の軸が1つに決定できません:')
		...
		for node in self :
			...
			if col_axis_member_list != None and col_axis_id != None:
				...
				for col_axis_member in col_axis_member_list :
					...
					logger.debug('メンバー:' + col_axis_member.get_jp_label() + ',' + col_axis_member.get_id())
					...
					for context in member_context_list :
						...
						logger.debug('[' + context.get_name() + ']'+ '[' + data + ']')
...
class XBRLStructureNode(LinkBaseNode):
	...
	def __str__(self) :
		...
		if xbrl_data != None :
			...
			if xbrl_data.get_data_dimension() == '1d' :

				xbrl_data_str = str(xbrl_data.get_data())
